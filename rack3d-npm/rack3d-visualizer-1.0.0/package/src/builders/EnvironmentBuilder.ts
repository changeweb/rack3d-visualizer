import type { IThemeScene } from '../types'
import type { ILightingOptions, IRoomOptions, ICustomLight } from '../types'
import type { Wall, Pillar } from '../types'

interface ExtendedRoomOptions extends IRoomOptions {
  walls?: Wall[]
  pillars?: Pillar[]
}
...
export class EnvironmentBuilder {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  buildLightsOnly(scene: any, lightingOptions: ILightingOptions, sceneTheme: IThemeScene): void {
    this._buildLights(scene, lightingOptions, sceneTheme, this._roomH ?? 35)
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  build(scene: any, roomOptions: ExtendedRoomOptions, lightingOptions: ILightingOptions, sceneTheme: IThemeScene): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildFloor(scene: any, W: number, D: number, cx: number, cz: number, ts2: number, ts: IThemeScene, ro: ExtendedRoomOptions): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildFloorTiles(scene: any, W: number, D: number, cx: number, cz: number, ts2: number, ts: IThemeScene): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildCeiling(scene: any, W: number, D: number, H: number, cx: number, cz: number, ts: IThemeScene): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildWalls(scene: any, W: number, D: number, H: number, cx: number, cz: number, ts: IThemeScene, ro: ExtendedRoomOptions): void {
    if (ro.walls && ro.walls.length > 0) {
      ro.walls.forEach(wall => {
        ...
        const color = wall.color
          ...
          : (ro.wallColor ? (typeof ro.wallColor === 'string' ? parseInt((ro.wallColor as string).replace('#', ''), 16) : ro.wallColor) : ts.wallColor)
    ...
    const baseColor = ro.wallColor
      ? (typeof ro.wallColor === 'string' ? parseInt((ro.wallColor as string).replace('#', ''), 16) : ro.wallColor)
      : ts.wallColor
    const wallMat = new this.THREE.MeshStandardMaterial({
      color: baseColor as number,
      roughness: 0.8,
      metalness: 0.05,
      side: this.THREE.DoubleSide
    })
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildPillars(scene: any, ro: ExtendedRoomOptions): void {
    ;(ro.pillars ?? []).forEach((p: Pillar) => {
      ...
      const color = p.color
        ...
        : 0x2a3a4a
      const mat = new this.THREE.MeshStandardMaterial({ color: color as number, roughness: 0.7, metalness: 0.3 })
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildStripLights(scene: any, W: number, D: number, H: number, cx: number, cz: number, ts: IThemeScene, ro: ExtendedRoomOptions, lo: ILightingOptions): void {
    ...
    if (ts.skipSkybox) {
      ...
      for (let ci = 0; ci < cols; ci++) {
        for (let ri = 0; ri < rows; ri++) {
          ...
          const spot = new this.THREE.SpotLight(0xfff8f0, overheadInt * 3, H * 1.8, Math.PI / 4, 0.5, 1)
      ...
      ;[-7, -2, 3, 8].forEach((z: number) => {
        ;[-5.5, 0, 5.5].forEach((x: number) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildBaseboardLights(scene: any, W: number, D: number, cx: number, cz: number, ts: IThemeScene, ro: ExtendedRoomOptions): void {
    ...
    ;[-W / 2 + cx, W / 2 + cx].forEach((x: number) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildSkyboxAndSun(scene: any): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _buildLights(scene: any, lo: ILightingOptions, ts: IThemeScene, roomH?: number): void {
    ...
    ;(lo.customLights ?? []).forEach((cl: ICustomLight) => {
      const color = typeof cl.color === 'string'
        ? parseInt((cl.color as string).replace('#', ''), 16)
        : (cl.color ?? 0xffffff)
      ...
      if (cl.type === 'point') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const pl = new this.THREE.PointLight(color, intensity, cl.distance ?? 0, (cl as any).decay ?? 1)
        pl.position.set(x, y, z)
        scene.add(pl)
      } else if (cl.type === 'ceiling') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const spot = new this.THREE.SpotLight(color, intensity, H * 1.8, (cl as any).angle ?? Math.PI / 4, (cl as any).penumbra ?? 0.5, (cl as any).decay ?? 1)
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const spot = new this.THREE.SpotLight(color, intensity, cl.distance ?? 0, (cl as any).angle ?? Math.PI / 6, (cl as any).penumbra ?? 0.3, (cl as any).decay ?? 1)
        spot.position.set(x, y, z)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        spot.target.position.set((cl as any).tx ?? x, (cl as any).ty ?? 0, (cl as any).tz ?? z)
        scene.add(spot)
        scene.add(spot.target)
      } else {

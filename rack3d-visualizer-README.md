...
```jsx
import RackViewer from 'rack3d-visualizer/examples/react/RackViewer';
...
### useEffect mount (manual)

```jsx
import { useEffect, useRef } from 'react';
import { Rack3DVisualizer } from 'rack3d-visualizer';

export function RackEmbed({ data, theme = 'dark' }) {
  ...
  useEffect(() => {
    ...
    return () => viz.current.destroy();   // ← always clean up
  }, []);

  useEffect(() => { viz.current?.setTheme(theme); }, [theme]);
  useEffect(() => { viz.current?.setData(data);   }, [data]);
